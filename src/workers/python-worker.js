...
async function waitPyodideReady() {
  try {
    ...
    postmessage("finished", [{ color: "red", msg: ["Failed to load Pyodide:", error.message] }]);
  }
}
...
self.onmessage = async (event) => {
  ...
  if (!pyodide) {
    try {
      ...
      postmessage("finished", [{ color: "red", msg: ["Failed to initialize Pyodide:", error.message] }]);
      return;
    }
  }
  ...
  // run the python code
  if (!code) {
    postmessage("finished", [{ color: "red", msg: ["Python code is empty"] }]);
  } else {
    try {
